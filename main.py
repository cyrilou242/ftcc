...
COMPRESSOR_PROVIDERS = {
    "DEFAULT_ZSTD": lambda: ZstdCompressor()
}
...
@click.option("--top_k_accuracy",
              ...
              default=["1"])
def run_experiment(dataset, compressor, top_k_accuracy):
    ...
    for k in top_k_accuracy:
        for c in compressor:
            method_result = {"Method": f"FFTC {c} top_{k} accuracy"}
            speed_result = {"Method": f"FFTC {c} top_{k} accuracy"}
            for d in dataset:
                ...
                classifier = CompressorClassifier(lambda: ZstdCompressor(), k)
                ...
                print(f"Accuracy on dataset {d}: {accuracy * 100}%. \nTraining time: {training_time}s. \nPrediction times: p50: {np.percentile(run_times_millis, 50)}ms, p90: {np.percentile(run_times_millis, 90)}ms, p99: {np.percentile(run_times_millis, 99)}ms.")
                method_result[d] = accuracy
                speed_result[d] = f"Training: {round(training_time, 1)}s." + f"Inference: p50: {round(np.percentile(run_times_millis, 50),3)}ms,p90: {round(np.percentile(run_times_millis, 90), 3)}ms,p99: {round(np.percentile(run_times_millis, 99),3)}ms"
